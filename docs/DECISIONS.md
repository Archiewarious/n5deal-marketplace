...
The README is the short version: what this is, how to run it, what to click. This file is the long
one — every decision that was not obvious, written down with the reasoning and, where there was
one, the measurement that settled it.

It is longer than a README should be, which is exactly why it is not the README.
